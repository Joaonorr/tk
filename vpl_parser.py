...
class VplParser:
    ...
    @staticmethod
    class CaseData:
        def __init__(self, case = "", input = "", output = "", grade: Optional[int] = None):
            self.case : str = case
            self.input : str = VplParser.finish(input)
            self.output : str = VplParser.unwrap(VplParser.finish(output))
